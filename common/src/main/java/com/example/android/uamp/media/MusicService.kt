...
import com.google.android.exoplayer2.upstream.DataSpec.HTTP_METHOD_GET
...
open class MusicService : MediaBrowserServiceCompat() {
    ...
    private fun getMediaSources(metadataList: List<MediaMetadataCompat>): List<MediaSource> {
        val mediaSources = ArrayList<MediaSource>()
        metadataList.forEach { mediaData ->
            val resolver = ResolvingDataSource.Resolver {
                var dataSpec: DataSpec? = null
                if (mediaData.downloadStatus == MediaDescriptionCompat.STATUS_DOWNLOADED) {
                    val url = getTrackPath(mediaData.id!!) ?: it.uri.toString()
                    ...
                    runBlocking {
                        val response = serviceConnection.getStreamingUrl(mediaData.id!!)
                        dataSpec = if (response.isSuccessful) {
                            DataSpec.Builder()
                                .setUri(Uri.parse(response.body()!!.stream.url))
                                .setPosition(it.position)
                                .build()
                        } else {
                            it
...
//                .createMediaSource()
    ...
    private fun getTrackPath(trackId: String?): String? {
        val file = File(File(filesDir, "offline"), "track_$trackId.mp3")
        return if (file.exists()) file.absolutePath else null
    }

    private fun getMediaSource(mediaData: MediaMetadataCompat): MediaSource {
        val resolver = ResolvingDataSource.Resolver {
            var dataSpec: DataSpec? = null
            runBlocking {
                val response = serviceConnection.getStreamingUrl(mediaData.id!!)
...
//        }
        ...
        serviceConnection = provideMusicServiceConnection(this)
        serviceConnection.mediaUpdateListener = object : MediaUpdateListener {
            override fun onMediaAdded(parentId: String, list: List<MediaMetadataCompat>) {
                currentPlaylistItems = mediaSource.toList()
                notifyChildrenChanged(parentId)
            }

            override fun onMediaRemoved(parentId: String) {
                currentPlaylistItems = mediaSource.toList()
                notifyChildrenChanged(parentId)
            }

            override fun addToNext(
                parentId: String,
                mediaMetadataCompat: MediaMetadataCompat,
                position: Int
            ) {
                currentPlaylistItems = mediaSource.toList()
                updatePlaylist(mediaMetadataCompat, position)
                notifyChildrenChanged(parentId)
            }
            ...
            override fun addToQueue(
                ...
                currentPlaylistItems = mediaSource.toList()
                updatePlaylist(mediaMetadataCompat)
                notifyChildrenChanged(parentId)
            }
        }
        mediaSource = serviceConnection.jsonSource
...
//            exoPlayer.prepare(mediaSource)
            exoPlayer.setMediaSources(getMediaSources(metadataList))
            exoPlayer.prepare()
    ...
    private fun updatePlaylist(mediaMetadataCompat: MediaMetadataCompat) {
        if (currentPlayer == exoPlayer) {
            exoPlayer.addMediaSource(getMediaSource(mediaMetadataCompat))
        } else {
            castPlayer!!.loadItem(mediaMetadataCompat.toMediaQueueItem(), 0)
        }
    }

    private fun updatePlaylist(mediaMetadataCompat: MediaMetadataCompat, position: Int) {
        if (currentPlayer == exoPlayer) {
            exoPlayer.addMediaSource(position, getMediaSource(mediaMetadataCompat))
        } else {
            castPlayer!!.loadItem(mediaMetadataCompat.toMediaQueueItem(), 0)
        }
    }
    ...
    private fun switchToPlayer(previousPlayer: Player?, newPlayer: Player) {
        ...
        serviceConnection.setPlayer(currentPlayer)
