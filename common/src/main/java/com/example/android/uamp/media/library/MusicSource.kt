...
interface MusicSource : Iterable<MediaMetadataCompat> {
     ...
     */
    suspend fun load(list: List<JsonMusic>)

    suspend fun newDataLoad(
        list: List<JsonMusic>,
        callback: ((list: List<MediaMetadataCompat>) -> Unit)
    )

    suspend fun addToNextData(
        jsonMusic: JsonMusic,
        currentPlayingPosition: Int,
        callback: ((Boolean, Int, mediaMetadataCompat: MediaMetadataCompat) -> Unit)
    )

    suspend fun addToQueueData(
        jsonMusic: JsonMusic,
        callback: ((mediaMetadataCompat: MediaMetadataCompat) -> Unit)
    )

    suspend fun replaceDataLoad(list: List<JsonMusic>)

    suspend fun updateDataLoad(jsonMusic: JsonMusic, mediaId: String)

    suspend fun removeDataLoad(controller: MediaControllerCompat, list: List<JsonMusic>)
