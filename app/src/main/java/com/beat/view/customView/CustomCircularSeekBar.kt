...
class CustomCircularSeekBar : View {
    ...
    interface OnSeekArcChangeListener {
         ...
         */
        fun onStartTrackingTouch(seekArc: CustomCircularSeekBar?)
         ...
         * The SeekArc in which the touch gesture began
         */
        fun onStopTrackingTouch(seekArc: CustomCircularSeekBar?)
    }
    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyle: Int) {
        ...
        var arcColor: Int = res.getColor(R.color.progress_gray_dark)
        var progressColor: Int = res.getColor(R.color.colorPrimary)
        ...
        if (attrs != null) {
            ...
            isEnable = a.getBoolean(R.styleable.SeekArc_enabled, isEnable)
    ...
    override fun onDraw(canvas: Canvas) {
        if (!isClockwise) {
            canvas.scale((-1).toFloat(), 1F, mArcRect.centerX(), mArcRect.centerY())
        }
        ...
        mArcPaint?.let {
            canvas.drawArc(
                mArcRect, arcStart.toFloat(), arcSweep.toFloat(), false,
                it
            )
        }
        mProgressPaint?.let {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        mThumbXPos =
            (mArcRadius * Math.cos(Math.toRadians(arcStart.toDouble()))).toInt()
        mThumbYPos =
            (mArcRadius * Math.sin(Math.toRadians(arcStart.toDouble()))).toInt()
    ...
    private fun updateThumbPosition() {
        ...
        mThumbXPos =
            (mArcRadius * Math.cos(Math.toRadians(thumbAngle.toDouble()))).toInt()
        mThumbYPos =
            (mArcRadius * Math.sin(Math.toRadians(thumbAngle.toDouble()))).toInt()
    }
    ...
    companion object {
        private val TAG = CustomCircularSeekBar::class.java.simpleName
